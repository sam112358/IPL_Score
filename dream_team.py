...
for i in range(1,61):
    ...
    for j in range(0, 22):
        if player_role[j] == 'Wicket Keeper' and no_wicket_keepers == 0:
    ...
    for j in range(0,22):
        if total_dream_players < 11:
            if player_role[j] == 'Batsman' and no_batsmen < 6 and player_name[j] not in dream_team:
                dream_team.append(player_name[j])
                analysis.write(a, 0, mno)
                analysis.write(a, 1, player_name[j])
                analysis.write(a, 2, player_role[j])
            ...
            if player_role[j] == 'Bowler' and no_bowlers < 6 and player_name[j] not in dream_team:
                dream_team.append(player_name[j])
                analysis.write(a, 0, mno)
                analysis.write(a, 1, player_name[j])
                analysis.write(a, 2, player_role[j])
